...
import { sha1 } from "https://x.kite.run/sha1@v1.0.3/mod.ts";
import titleCase from "https://x.kite.run/case@v2.0.0/titleCase.ts";
...
class Helpers {
	...
	number = (a: string) => {
		let n = Number(a);
		if (Number.isNaN(n)) {
			throw new Error(`could not parse "${a}" as number`);
		}
		return a;
	};
...
String.prototype.sha1sum = function () {
	const k = "sha1sum";
	return isFuture(this)
		? appendOp(this, k)
		: (h[k](this as string) as string);
};
