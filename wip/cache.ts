		...
		return cacache.get
			...
			.then((info) => {
				return (
					info &&
					cacache.get
						.hasContent(this._path, info.integrity, opts)
					...
					if (opts.memoize !== false && info.size > MAX_MEM_SIZE) {
						...
						body = new stream.Readable({
							read() {
								...
								readOnce = true
								cacache.get
									.byDigest(cachePath, info.integrity, {
										memoize: opts.memoize,
									})
									.then(
										(data) => {
			...
			(chunk, enc, cb) => {
				if (!cacheTargetStream) {
					if (fitInMemory) {
						...
						cacheTargetStream = cacache.put.stream(
							cachePath,
							cacheKey(req),
							cacheOpts
						)
