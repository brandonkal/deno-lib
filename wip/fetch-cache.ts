 ...
 * @license ISC
 * @description Deno cache for fetch. Overrides window.fetch with a cached version respecting HTTP.
 * Contains code obtained via the ISC license: Copyright (c) npm, Inc.
 */
...
function isStale(req, res) {
	...
	const responseTime =
		res.headers.get('x-local-cache-time') || res.headers.get('date') || 0
		...
		.then((condRes) => {
			...
			if (condRes.status === 304) {
				...
				return opts.cacheManager.put(req, condRes, opts).then((newRes) => {
					newRes.headers = new fetch.Headers(
...
function isHeaderConditional(headers) {
	...
	return Object.keys(headers).some(
		(h) => modifiers.indexOf(h.toLowerCase()) !== -1
	)
}
