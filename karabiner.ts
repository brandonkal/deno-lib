...
export function rule(
	...
	if (conditions) {
		if (!Array.isArray(conditions)) {
			throw new Error(
				`Conditions must be an array. Got ${str(conditions)}`,
			);
		}
		...
		outConditions = conditions.map((condition) => {
			if (isVarShorthand(condition)) {
				...
				throw new Error(
					`Condition is invalid. Expected [string, number] or valid type. Got ${
						str(
							condition,
						)
					}`,
	...
	const outFrom = typeof from[0] !== "string"
		? from[0]
		: genFromEvent(from as keys.Key[]);
	const outTo = to.map(
		(key): IToEvent => {
	...
	return {
		conditions: outConditions.length
			? outConditions ?? undefined
			: undefined,
		from: outFrom,
...
function genFromEvent(keyCodes: keys.Key[]): IFromEvent {
	...
	if (!standardKeys.length) {
		...
		if (out.modifiers?.mandatory?.length) {
			...
			throw new Error(
				`A from rule requires a non-optional key but got ${
					str(keyCodes)
				}`,
			);
		}
...
function genToEvent(keyCodes: keys.Key[]): IToEvent {
	...
	keyCodes.forEach((key) => {
		if (keys.isToModifierKey(key)) {
			...
			throw new Error(
				`Invalid key code for to event. Got ${str(key)} from ${
					str(keyCodes)
				}`,
			);
		}
