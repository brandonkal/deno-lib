...
export function parseRequest(
	...
	if (!parsed.params.algorithm) {
		throw new Error("algorithm was not specified");
	}

	if (!parsed.params.signature) {
		throw new Error("signature was not specified");
	}

	// Check the algorithm against the official list
	const alg =
		(parsed.params.algorithm = parsed.params.algorithm.toUpperCase());
	if (alg !== "HMAC-SHA256") {
	...
	for (let i = 0; i < parsed.params.headers.length; i++) {
		...
		if (h === "request-line") {
			...
			parsed.signingString += "(request-target): " +
	...
	if (req.headers.date || req.headers["x-date"]) {
		...
		if (skew > opts.clockSkew * 1000) {
			throw new Error(
				`clock skew of ${
					skew / 1000
				}s was greater than ${opts.clockSkew}s`,
			);
		}
	}
...
export function verifySignature(parsed: ParsedRequest, key: string): boolean {
	...
	if (parsed.params.algorithm === "HMAC-SHA256") {
		const hmac = encodeBase64(
			new HmacSha256(key).update(text).arrayBuffer(),
		);
		if (
